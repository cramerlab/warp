...
class EstimatorV2(object):
  ...
  def __init__(self, model_fn, model_dir=None, config=None, params=None):
    ...
    """
    EstimatorV2._assert_members_are_not_overridden(self)
  ...
  def train_one_step(self,
    ...
    saving_listeners = _check_listeners_type(saving_listeners)
    loss = self._train_model_one_step(input_fn, hooks, saving_listeners)
    logging.info('Loss for final step: %s.', loss)
    return self
  ...
  def _assert_members_are_not_overridden(self):
    ...
    estimator_members = set([m for m in EstimatorV2.__dict__.keys()
    ...
    overridden_members = [
        ...
        if EstimatorV2.__dict__[m] != self.__class__.__dict__[m]]
  ...
  def export_savedmodel(self, 
    ...
    export_dir = os.path.join(export_dir_base, export_name)
    ...
    with ops.Graph().as_default() as g:
      ...
      with tf_session.Session() as session:
        ...
        estimator_spec_train = self._call_model_fn(features=serving_input_receiver.features,
                                                   labels=None,
                                                   mode=model_fn_lib.ModeKeys.TRAIN,
                                                   config=self.config)

        # Build the SignatureDefs from receivers and all outputs
        signature_def_map = build_all_signature_defs(serving_input_receiver.receiver_tensors,
                                                     estimator_spec_train.export_outputs,
                                                     serving_input_receiver.receiver_tensors_alternatives)

        saver_for_restore = estimator_spec_train.scaffold.saver or saver.Saver(sharded=True)
        saver_for_restore.restore(session, checkpoint_path)

        local_init_op = (estimator_spec_train.scaffold.local_init_op or
                         monitored_session.Scaffold.default_local_init_op())

        builder.add_meta_graph_and_variables(session, ['train'],
    ...
    with ops.Graph().as_default() as g:
      ...
      with tf_session.Session() as session:
        ...
        estimator_spec_serve = self._call_model_fn(features=serving_input_receiver.features,
                                                   labels=None,
        ...
        # Build the SignatureDefs from receivers and all outputs
        signature_def_map = build_all_signature_defs(serving_input_receiver.receiver_tensors,
                                                     estimator_spec_serve.export_outputs,
                                                     serving_input_receiver.receiver_tensors_alternatives)

        saver_for_restore = estimator_spec_serve.scaffold.saver or saver.Saver(sharded=True)
        saver_for_restore.restore(session, checkpoint_path)

        local_init_op = (estimator_spec_serve.scaffold.local_init_op or
                         monitored_session.Scaffold.default_local_init_op())
                         
        my_saver = tf.train.Saver()
        saver_def = my_saver.as_saver_def()
  ...
  def _train_model_one_step(self, input_fn, hooks, saving_listeners):
    worker_hooks = []
    with ops.Graph().as_default() as g, g.device(self._device_fn):
      random_seed.set_random_seed(self._config.tf_random_seed)
      global_step_tensor = self._create_and_assert_global_step(g)
